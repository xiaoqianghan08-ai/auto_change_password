...
5. Infers the target type from the application name when `--target auto` is used, then brings the target application window to the foreground:
   - `desktop`: use `win32gui` window enumeration and activation; in batch mode, prefer an already-open window before launching. When launching, resolve executable names such as `QQ.exe`/`QQScLauncher.exe`, Windows App Paths registry entries, known install paths, and Start Menu/Desktop shortcuts instead of blindly running `start <应用名称>`.
